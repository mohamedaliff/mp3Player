...
Public Class AudioFile
    ...
    Public Sub FastForward(ByVal currentDuration As Integer)

        If (IsPaused = False) Then

            mciSendString("seek audiofile to " + (currentDuration + 5000).ToString, Nothing, 0, IntPtr.Zero)
            mciSendString("play audiofile", Nothing, 0, IntPtr.Zero)

        End If
    End Sub

    Public Sub Rewind(ByVal currentDuration As Integer)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    ReadOnly Property SecondsPlayed() As Integer
        Get
            Dim buf As String = Space(255)
            mciSendString("status audiofile position", buf, 255, IntPtr.Zero)
